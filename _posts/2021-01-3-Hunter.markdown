...
9- `(When did the port scan start and end)`
من المعروف أن أداة التقرير الناتج عن أداة  `(zenmap)` سوف نجد وقت بدء عملية المسح و كم المدة المستغرقة في البحث. 
...
10- `(How many ports were scanned)`
في نفس الملف المذكور ب السؤال السابق سنجد كم `(Ports)` تم مسحة 
![](https://i.ibb.co/7JkQxT7/q11.png)

11- `(What ports were found “open”)`
أيضا بنفس الملف سوف نجد  كم `(Ports)` تم  ايجادة في حالة `(Open)` 
